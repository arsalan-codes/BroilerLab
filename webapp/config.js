/* Arian runtime configuration — loaded before app/auth/device-panel.
   API base resolution order:
     1. window.ARIAN_API_OVERRIDE  (manual override, e.g. ?api= or injected script)
...
(function () {
  ...
  var base = "";
  try {
    if (window.ARIAN_API_OVERRIDE) base = window.ARIAN_API_OVERRIDE;
  ...
  window.ARIAN_API = String(base).replace(/\/+$/, "");
})();
